...
function Edit(){
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            const change = await axios.put("http://localhost:5000/edit_copies", 
            {"title": title, 
            "copies": copies
        });
    ...
    {updated ?
        <div>
            <h1 className="display-3 text-danger font-weight-bold">Copies Updated Successfully </h1>
            <a className="btn btn-warning btn-lg col col-lg-2 left-align font-weight-bold" href="/books" role="button">View</a>
        </div> :
        <div>
            <h1 className="display-3 text-danger font-weight-bold">Edit Available Books Copies</h1>
            ...
            <p></p>
            <label htmlFor="copies" className="font-weight-bold ">Updated Number of Copies</label>
            <input onChange={handleCopiesChange} type="text" name="copies" className="form-control" id="copies" value={copies} />
            <small id="emailHelp" className="form-text text-muted">Enter new updated number of copies.</small>
            <p></p>
            <button type="submit" className="btn btn-warning btn-lg font-weight-bold">Update</button>
            </div>
            </form>
        </div>
    }
